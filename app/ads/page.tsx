  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-gray-500 text-sm mb-6">過去30日のCPC・CTR・CPAをAPI連携で自動取得</p>
              ...
              <PlatformCard
                ...
                envHelp={
                  <ol className="text-xs text-gray-500 space-y-1 list-decimal list-inside">
                    ...
                    <li>Vercel 環境変数に <code className="text-blue-700">META_ACCESS_TOKEN</code> と <code className="text-blue-700">META_AD_ACCOUNT_ID</code> を追加</li>
                    <li>「同期する」ボタンを押す</li>
                  </ol>
                }
              />
...
function PlatformCard({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4">
      ...
      {!isConnected ? (
        <div>
          <p className="text-sm text-gray-700 mb-3">API連携が未設定です。以下の手順でセットアップしてください。</p>
          {envHelp}
          {status === 'error' && cred.last_error && (
            <p className="text-xs text-red-700 mt-2">最新エラー: {cred.last_error}</p>
          )}
          <button
            onClick={onSync}
            disabled={syncing}
            className="mt-3 w-full px-3 py-2 rounded text-sm bg-gray-50 border border-gray-300 hover:bg-gray-100 disabled:opacity-50"
          >
            {syncing ? '確認中…' : '接続テスト'}
          </button>
        </div>
